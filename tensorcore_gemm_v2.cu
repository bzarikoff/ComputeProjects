...
#define BM (BLOCK_TILES_M * WM)   // block tile rows = 128
#define BN (BLOCK_TILES_N * WN)   // block tile cols = 128
...
__global__ void gemm_tensorcore_v2(const half* A, const half* B, float* C, int N) {
...
int main(int argc, char** argv) {
    ...
    printf("GEMM (Tensor Core v2, fragment tiling): N = %d, CPU verification: %s\n", N, do_cpu_check ? "on" : "off (N too large)");
    ...
    gemm_tensorcore_v2<<<gridDim, blockDim>>>(d_A, d_B, d_C, N);
