...
#define BK WMMA_K                      // K-chunk = one wmma step per shared-mem tile
...
__global__ void gemm_tensorcore(const half* A, const half* B, float* C, int N) {
    ...
    for (int t = 0; t < numTiles; ++t) {
        ...
        // One wmma step per k-chunk (BK == WMMA_K here, so this runs once
        // per outer loop iteration -- a larger BK would loop this inner part).
        wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half, wmma::row_major> a_frag;
        wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, half, wmma::row_major> b_frag;

        wmma::load_matrix_sync(a_frag, &As[warpRow * WMMA_M][0], BK);
        wmma::load_matrix_sync(b_frag, &Bs[0][warpCol * WMMA_N], BN);
